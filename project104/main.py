...
def mode(numbers, ranges: list) -> float:
    ...
    for value, occurs in numbers.items():
        for range in ranges:
            range_low = int(range.split("-")[0])
            range_high = int(range.split("-")[1])
            
            if range_low<value<range_high:
                ranges_dict[range] += occurs

    range, occurs = 0,0
    for range, occurs in ranges_dict.items():
        if occurs > occurs:
            _range, _occurs = [int(range.split("-")[0]),int(range.split("-")[1])], occurs
